...
const dataUrl = await p.evaluate(async () => {
  const img = new Image(); img.src = '/assets/game/rakshasa_src.png?' + Date.now(); await img.decode();
  const W = img.width, H = img.height;
  const c = document.createElement('canvas'); c.width = W; c.height = H;
  const ctx = c.getContext('2d'); ctx.drawImage(img, 0, 0);
  const im = ctx.getImageData(0, 0, W, H); const d = im.data;
  const idx = (x, y) => (y * W + x) * 4;
  const isChecker = (i) => { const r = d[i], g = d[i + 1], bl = d[i + 2]; const mx = Math.max(r,g,bl), mn = Math.min(r,g,bl); return (mx - mn) <= 14; };
...
fs.writeFileSync('public/assets/game/rakshasa.png', Buffer.from(base64, 'base64'));
