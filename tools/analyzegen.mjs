...
const [file, rowsArg] = process.argv.slice(2);
const ROWS = parseInt(rowsArg, 10);
const b = await chromium.launch();
const p = await b.newPage();
await p.goto('http://localhost:5173/', { waitUntil: 'domcontentloaded' });
const report = await p.evaluate(async ({ file, ROWS }) => {
  ...
  // Background: neutral checker (light or dark) OR near-black grid lines.
  ...
  for (let r=0;r<ROWS;r++){
    ...
    results.push({row:r, big:big.length, figs: big.map(x=>({x:x.minx,X:x.maxx,w:x.w,h:x.h,n:x.cnt}))});
  }
