#!/usr/bin/env bash
#
# Puts the game on a public URL you can open on a phone.
#
#   tools/serve.sh            the game, from the vite dev server
#   tools/serve.sh stop       take it down again
#
...
# slots. This script finds one nothing is using and starts the server on
# that, instead of starting the server where it likes and trying to publish
# the port afterwards. That inversion is the entire trick. Ports are a
# Codespace-wide resource, so this happily reuses one already opened for
# another project in the same Codespace.
#
...
#
# State lives under $HOME rather than /tmp for the same reason: /tmp does not
# survive the restart.

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
STATE="$HOME/.hqs-serve"
...
if ! curl -sS -o /dev/null --max-time 5 "http://localhost:$PORT/" 2>/dev/null; then
    echo "Server never came up on $PORT - see $STATE/game.log" >&2
    exit 1
fi
