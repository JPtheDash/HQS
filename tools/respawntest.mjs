...
const st = async () => p.evaluate(() => { const g = window.game.scene.getScene('GameScene'); const pl = g.player; return { x: Math.round(pl.x), y: Math.round(pl.y), hp: g.health, fin: g.finished }; });
console.log('start', JSON.stringify(await st()));
for (let k = 0; k < 4; k++) {
  await p.evaluate(() => { const pl = window.game.scene.getScene('GameScene').player; pl.setPosition(pl.x, 1500); pl.setVelocityY(400); });
  await p.waitForTimeout(500);
  console.log('after fall ' + k, JSON.stringify(await st()));
}
