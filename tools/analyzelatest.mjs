// Analyze latest_src.png: report dimensions, and per-row connected-component
// figures (count, bbox, size) after removing the light checker background.
...
const report = await p.evaluate(async () => {
  ...
  const W = img.width, H = img.height, rows = 4;
  ...
  for (let r=0;r<rows;r++){
    ...
    for (let y=y0;y<y1;y++) for (let x=0;x<W;x++){
      ...
      while(q.length){const cy=q.pop(),cx=q.pop();cnt++;if(cx<minx)minx=cx;if(cx>maxx)maxx=cx;if(cy<miny)miny=cy;if(cy>maxy)maxy=cy;
        for(let dy=-1;dy<=1;dy++)for(let dx=-1;dx<=1;dx++){const nx=cx+dx,ny=cy+dy;if(nx<0||ny<y0||nx>=W||ny>=y1)continue;const nl=(ny-y0)*W+nx;const ni=(ny*W+nx)*4;if(d[ni+3]>20&&lab[nl]===-1){lab[nl]=comps.length;q.push(nx,ny);}}}
      comps.push({minx,maxx,miny,maxy,cnt,w:maxx-minx+1,h:maxy-miny+1});
    }
    comps.sort((a,b)=>a.minx-b.minx);
    const big = comps.filter(x=>x.cnt>1500);
    results.push({row:r, count: big.length, figs: big.map(x=>({x:x.minx,X:x.maxx,w:x.w,h:x.h,n:x.cnt}))});
  }
  return {W,H,rowH, results};
});
